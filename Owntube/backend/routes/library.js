...
// ════════════════════════════════════════════════════════════
// ДОБАВЛЕНО: СТАТИЧНЫЕ ОБЛОЖКИ ВИДЕО (jpg/png/webp)
// Раньше у видео вообще не было обложек с диска — фронтенд (см.
// showEnd()/renderRec() в app.js) уже умел показывать v.coverUrl
// для серверных видео, но бэкенд никогда его не отдавал. Работает
// по тому же принципу, что и ANIM_DIR выше: имя файла-картинки
// сопоставляется с именем видео через norm().
// ════════════════════════════════════════════════════════════
const VIDEO_COVERS_DIR = (() => {
  try { return require('../config').VIDEO_COVERS_DIR; } catch (e) { return null; }
})() || String.raw`F:\(8 фелиал АДА)\sLOW\vid cover`;

const VIDEO_COVER_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp'];
let videoCoverMap = new Map();
const VIDEO_COVER_CACHE_TTL = 60000;

async function refreshVideoCoverMapAsync() {
  const next = new Map();
  if (!fs.existsSync(VIDEO_COVERS_DIR)) {
    console.warn('[обложки видео] папка не найдена:', VIDEO_COVERS_DIR);
    videoCoverMap = next;
    return;
  }
  try {
    async function walk(dir) {
      ...
      for (const e of entries) {
        ...
        if (e.isDirectory()) { await walk(abs); continue; }
        const ext = path.extname(e.name).toLowerCase();
        if (!VIDEO_COVER_EXTENSIONS.includes(ext)) continue;
        const nameNorm = norm(path.basename(e.name, ext));
        const relFromCovers = path.relative(VIDEO_COVERS_DIR, abs).replace(/\\/g, '/');
        next.set(nameNorm, {
          filename: e.name,
          relPath: relFromCovers,
          coverUrl: '/cover/video/' + encodeURIComponent(relFromCovers).replace(/%2F/g, '/'),
        });
      }
    }
    await walk(VIDEO_COVERS_DIR);
    videoCoverMap = next;
    console.log(`[обложки видео] найдено ${videoCoverMap.size} обложек в ${VIDEO_COVERS_DIR}`);
  } catch (err) {
    console.error('[обложки видео] ошибка сканирования:', err.message);
  }
}

function findCoverForVideo(name) {
  if (!name) return null;
  return videoCoverMap.get(norm(name)) || null;
}

refreshVideoCoverMapAsync();
setInterval(refreshVideoCoverMapAsync, VIDEO_COVER_CACHE_TTL);
// ──────────────────── КОНЕЦ БЛОКА ОБЛОЖЕК ВИДЕО ─────────────────────
...
router.get('/library/video', async (req, res) => {
  try {
    ...
    const withUrls = items.map(it => {
      ...
      const cover = findCoverForVideo(it.name);
      return {
        ...it,
        vertical,
        ...
        // ДОБАВЛЕНО: см. блок "СТАТИЧНЫЕ ОБЛОЖКИ ВИДЕО" выше.
        coverUrl: cover ? cover.coverUrl : null,
      };
    });
...
router.use('/cover/video', (req, res, next) => {
  ...
  if (!resolved.startsWith(coversDirResolved + path.sep) && resolved !== coversDirResolved) {
    return res.status(403).end();
  }
...
// ============================================================
// ДОБАВЛЕНО: ЛАЙКИ/ИЗБРАННОЕ — привязаны к аккаунту (JWT req.user),
// а не к браузеру. Раньше "нравится" у треков хранилось только в
// localStorage/IndexedDB на фронте (см. TRACK_LIKES_KEY в app.js) —
// терялось при смене браузера/устройства даже под тем же логином.
// ============================================================
const { getLikes, setLike } = require('../lib/authStore');

router.get('/library/likes', (req, res) => {
  try {
    if (!req.user || !req.user.username) return res.status(401).json({ error: 'Unauthorized' });
    res.json({ likes: getLikes(req.user.username) });
  } catch (e) {
    console.error('[ОШИБКА /library/likes]', e);
    res.status(500).json({ error: 'Не удалось получить лайки' });
  }
});

router.post('/library/likes', (req, res) => {
  try {
    if (!req.user || !req.user.username) return res.status(401).json({ error: 'Unauthorized' });
    const { type, id, state } = req.body || {};
    if (!id || (type !== 'video' && type !== 'music')) {
      return res.status(400).json({ error: 'Некорректные данные (нужны type: video|music и id)' });
    }
    const likes = setLike(req.user.username, type, id, Number(state) || 0);
    if (!likes) return res.status(404).json({ error: 'Пользователь не найден' });
    res.json({ ok: true, likes });
  } catch (e) {
    console.error('[ОШИБКА POST /library/likes]', e);
    res.status(500).json({ error: 'Не удалось сохранить лайк' });
  }
});
