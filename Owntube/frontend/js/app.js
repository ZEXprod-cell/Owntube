...
const VS={
  ...
  load(id){
    ...
    // Раньше здесь blob-URL для больших файлов (>50MB, т.е. длинные видео)
    // отзывался принудительно через 8 секунд таймером — если видео не
    // успевало полностью догрузиться/забуфериться за это время (обычное
    // дело для длинных видео), URL становился невалидным прямо во время
    // просмотра и плеер намертво зависал. Отзыв URL для больших файлов
    // уже корректно выполняется в switchVid/resetApp/showDlPage при
    // реальном закрытии/смене видео — досрочный таймер не нужен.
    if(f.size>50*1024*1024){const u=URL.createObjectURL(f);return{url:u,type:'large'};}
...
let normalizationEnabled=localStorage.getItem('ot_normalization')!=='0'; // включена по умолчанию
...
async function renderAcmList(query=''){
  ...
  let items=[];
  try{
    // Используем window.owntubeAuthHeader() для авторизации
    const res=await fetchWithTimeout(`${SERVER_API}/library/anim-covers`, { headers: window.owntubeAuthHeader() }, 8000);
...
async function fetchVideoLibrary() {
  ...
  try {
    // Используем window.owntubeAuthHeader() через opts
    const data = await fetchJsonWithRetry(`${SERVER_API}/library/video`, { opts: { headers: window.owntubeAuthHeader() } });
...
function initPlayerControls(){
  ...
  // ── Watchdog от "мёртвых" зависаний на длинных видео ──
  // Раньше при обрыве/подвисании сети во время буферизации длинного видео
  // плеер мог зависнуть навсегда без единого признака ошибки. Здесь ловим
  // 'error' и длительный 'waiting' без прогресса и пробуем восстановиться
  // однократным reload с той же позиции; если не помогает — сообщаем.
  let stallTimer=null;
  let lastStallTime=-1;
  ...
  function armStallTimer(){
    ...
    stallTimer=setTimeout(()=>{
      if(player.paused||!curVidId)return;
      recoverFromStall();
    },15000);
  }
  function recoverFromStall(){
    const t=player.currentTime;
    if(t===lastStallTime && recoverAttempted){
      alert('Видео зависло из-за проблем с сетью/сервером. Попробуйте переключить видео заново.');
      recoverAttempted=false;lastStallTime=-1;
      return;
    }
    lastStallTime=t;recoverAttempted=true;
  ...
  player.addEventListener('waiting',armStallTimer);
  player.addEventListener('playing',()=>{clearStallTimer();recoverAttempted=false;});
  player.addEventListener('timeupdate',()=>{if(!player.paused)recoverAttempted=false;});
  player.addEventListener('error',()=>{if(!player.paused||curVidId)recoverFromStall();});
...
function updateBar(track){
  ...
  // Бар плеера — только статичная обложка (анимация в баре отключена)
  {
  ...
  // Динамическая обложка в панели — анимация ТЕКУЩЕГО ТРЕКА при проигрывании
  if (dynCover) {
    const panel = document.getElementById('trackPanel');
    if (panel?.classList.contains('show')) {
      const cw = document.getElementById('trackPanelInner')?.querySelector('.tp-cover-wrap');
      if (cw) {
        if (animCoversEnabled && track.animCoverUrl) {
          const acUrl2 = track.animCoverUrl.startsWith('/') ? (SERVER_API + track.animCoverUrl) : track.animCoverUrl;
          // Не перезапускаем анимацию если уже играет та же самая
          const existingV = cw.querySelector('video.anim-cover');
          const existingSrc = existingV ? (existingV.src || existingV.getAttribute('src') || '') : '';
          const isSameAnim = existingSrc && (existingSrc === acUrl2 || existingSrc.split('?')[0] === acUrl2.split('?')[0]);
          if (!isSameAnim) {
            cw.innerHTML = animCoverHtml(acUrl2, track.animCoverType, true);
            const v = cw.querySelector('video.anim-cover');
            if (v && !MP.audio.paused) v.play().catch(() => {});
          }
          // Если та же анимация — просто убеждаемся что она играет/паузит в такт аудио
        } else if (cu) {
          // Не заменяем существующую анимированную обложку статичной картинкой
          if (!cw.querySelector('.anim-cover')) {
            const existingImg = cw.querySelector('img');
            if (!existingImg || existingImg.src !== cu) {
              cw.innerHTML = `<img class="track-panel-cover" src="${cu}" alt="">`;
            }
          }
        } else {
          if (!cw.querySelector('.track-panel-cover-ph') && !cw.querySelector('.anim-cover')) {
...
async function mPlay(id) {
  ...
  // Нормализация — запускаем асинхронно, не блокируем воспроизведение
  applyNormalization(audioSrc, track);
  MP.trackId = id;
...
function pauseAnimCovers(){document.querySelectorAll('.tp-cover-wrap video.anim-cover').forEach(v=>v.pause());}
function playAnimCovers(){
  // Анимация в панели играет только если текущий трек из того же альбома
  const shouldPlay = _tpTrackInPanel();
  document.querySelectorAll('.tp-cover-wrap video.anim-cover').forEach(v=>{
    if(shouldPlay) v.play().catch(()=>{});
    else v.pause();
  });
}
function _tpTrackInPanel(){
  if(!tpAlbum || !MP.trackId) return false;
  // Раньше здесь было сравнение "artist|album" строк для серверных треков,
  // но t.album у трека и albumFilter, из которого строился tpAlbum, могли
  // отличаться (trim, регистр, пустая строка vs 'Без альбома') — из-за этого
  // сравнение почти всегда проваливалось и анимация не запускалась.
  // Список #tpTrackList всегда содержит data-tid для всех треков текущей
  // панели (локальных и серверных), поэтому надёжнее проверять через DOM.
  return !!document.querySelector(`#tpTrackList .tp-track[data-tid="${MP.trackId}"]`);
}
function syncAnimCovers(){
  const sel='.tp-cover-wrap video.anim-cover';
  const canPlay = !MP.audio.paused && _tpTrackInPanel();
  document.querySelectorAll(sel).forEach(v=>{if(canPlay)v.play().catch(()=>{});else v.pause();});
}
...
async function fetchMusicLibrary() {
  ...
  try {
    // Используем window.owntubeAuthHeader() через opts
    const data = await fetchJsonWithRetry(`${SERVER_API}/library/music`, {
      timeouts: [10000, 20000, 30000],
      opts: { headers: window.owntubeAuthHeader() }
    });
...
function showTP(artist,album,tracks){
  ...
  syncAnimCovers(); // вызываем после построения tpTrackList, чтобы _tpTrackInPanel() нашёл элементы
...
let aCtx,analyser,gainNode,eqCvs,eqCtx,eqData,eqAnim;
function initEQ(){
  if(aCtx)return;
  aCtx=new(window.AudioContext||window.webkitAudioContext)();
  gainNode=aCtx.createGain();gainNode.gain.value=1.0;
  ...
  // граф: src → gainNode → analyser → destination
  const src=aCtx.createMediaElementSource(MP.audio);
  src.connect(gainNode);gainNode.connect(analyser);analyser.connect(aCtx.destination);
...
// Целевой RMS уровень (~-14 LUFS). Gain ограничен до 3× чтобы не перегружать тихие треки.
const NORM_MAX_GAIN   = 3.0;
const NORM_MIN_GAIN   = 0.25;

async function calcNormGain(audioUrl) {
  const TARGET_RMS = 0.20; // ~-14 LUFS
  // initEQ уже вызван до нас, aCtx гарантированно существует

  // Пробуем: сначала весь файл (до 8MB), потом если упало — без Range (полный файл)
  const head = await fetch(audioUrl, { method: 'HEAD' }).catch(() => null);
  const total = head ? parseInt(head.headers.get('content-length') || '0') : 0;
  const limit = 8 * 1024 * 1024;

  const attempts = [];
  if (total > 0 && total <= limit) {
    attempts.push({}); // файл влезает — без Range
  } else if (total > limit) {
    attempts.push({ Range: `bytes=0-${limit - 1}` }); // обрезаем
    attempts.push({}); // fallback — полный файл
  } else {
    attempts.push({}); // размер неизвестен — пробуем без Range
  }
  ...
  for (const headers of attempts) {
    try {
      ...
      if (buf.byteLength < 16384) continue;

      // slice(0) — копируем буфер т.к. decodeAudioData его потребляет
      const decoded = await aCtx.decodeAudioData(buf.slice(0));
      const ch = decoded.getChannelData(0);

      const skip = Math.min(Math.floor(decoded.sampleRate * 0.5), ch.length >> 1);
      let sum = 0;
      const count = ch.length - skip;
      for (let i = skip; i < ch.length; i++) sum += ch[i] * ch[i];
      const rms = count > 0 ? Math.sqrt(sum / count) : 0;

      if (rms < 0.0001) return 1.0;
      const g = TARGET_RMS / rms;
      const clamped = Math.min(NORM_MAX_GAIN, Math.max(NORM_MIN_GAIN, g));
      console.debug(`[normalization] rms=${rms.toFixed(4)} gain=${clamped.toFixed(3)} (${(buf.byteLength/1024/1024).toFixed(1)}MB)`);
      return clamped;
    } catch(e) {
      console.debug(`[normalization] attempt failed: ${e.message}`);
    }
  }
  console.warn('[normalization] не удалось декодировать, gain=1.0');
  return 1.0;
}

async function applyNormalization(audioUrl, track) {
  initEQ(); // убеждаемся что gainNode создан
  if (!normalizationEnabled) {
    gainNode.gain.setTargetAtTime(1.0, aCtx.currentTime, 0.1);
    return;
  }

  // Если gain уже посчитан ранее и сохранён в метаданных — берём его сразу
  if (track && track.normGain && track.normGain > 0) {
    gainNode.gain.setTargetAtTime(track.normGain, aCtx.currentTime, 0.1);
    console.debug(`[normalization] cached gain=${track.normGain.toFixed(3)} for ${audioUrl.split('/').pop()}`);
    return;
  }

  // Считаем gain впервые
  const g = await calcNormGain(audioUrl);
  gainNode.gain.setTargetAtTime(g, aCtx.currentTime, 0.1);
  console.debug(`[normalization] computed gain=${g.toFixed(3)} for ${audioUrl.split('/').pop()}`);

  // Сохраняем в метаданные трека на сервере чтобы не пересчитывать
  if (track && track.id && g !== 1.0) {
    try {
      await fetch(SERVER_API + '/library/music/meta', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', ...window.owntubeAuthHeader() },
        body: JSON.stringify({ id: track.id, normGain: g })
      });
      // Обновляем локальный кэш
      if (serverMusicMap.has(track.id)) serverMusicMap.get(track.id).normGain = g;
    } catch(e) {
      console.warn('[normalization] не удалось сохранить gain:', e.message);
    }
  }
}
...
function setNormalization(enabled) {
  ...
  localStorage.setItem('ot_normalization', enabled ? '1' : '0');
  if (gainNode) {
    if (!enabled) {
      gainNode.gain.setTargetAtTime(1.0, aCtx.currentTime, 0.1);
    } else if (MP.trackId) {
      // пересчитываем для текущего трека
      const t = serverMusicMap.get(MP.trackId);
      if (t && t.streamUrl) applyNormalization(t.streamUrl.startsWith('/') ? SERVER_API + t.streamUrl : t.streamUrl);
    }
  }
}
// ───────────────────────────────────────────────────────────────────────────
...
function matchAnimForTrack(track) {
  ...
  // Только название трека (title / name) — альбом и исполнитель намеренно исключены.
  // Анимация с matchTypes только 'album' к треку не применяется.
  const titleKeys = [];
  if (track.title) titleKeys.push(track.title.toLowerCase().trim());
  if (track.name)  titleKeys.push(track.name.toLowerCase().trim());
  ...
  const tryFind = (onlyWebM) => {
    for (const ac of animCoversCache) {
      ...
      // Пропускаем анимации, которые сервер разметил только как альбомные
      const types = ac.matchTypes || [];
      if (types.length && !types.includes('title')) continue;

      const acName = (ac.name || '').toLowerCase().trim();
      // Только точное совпадение по названию трека
...
function matchAnimForAlbum(album, artist) {
  ...
  const q = album.toLowerCase().trim();
  ...
  const tryFind = (onlyWebM) => {
    for (const ac of animCoversCache) {
      ...
      // Пропускаем анимации, которые сервер разметил только как трековые
      const types = ac.matchTypes || [];
      if (types.length && !types.includes('album')) continue;

      const acName = (ac.name || '').toLowerCase().trim();
      // Только точное совпадение: имя анимации === название альбома
      if (acName === q) {
        return { animCoverUrl: ac.streamUrl.startsWith('/') ? SERVER_API + ac.streamUrl : ac.streamUrl, animCoverType: ac.type || 'image/gif' };
      }
      // Или сервер явно сопоставил альбом в matchedAlbums
      if (ac.matchedAlbums && ac.matchedAlbums.some(a => a.toLowerCase().trim() === q)) {
        return { animCoverUrl: ac.streamUrl.startsWith('/') ? SERVER_API + ac.streamUrl : ac.streamUrl, animCoverType: ac.type || 'image/gif' };
      }
    }
    return null;
